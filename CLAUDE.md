...
- **Marks and colour** (Tyler, 2026-09-17). Every invented company has a
  drawn mark, `private/marks/<slug>.svg`, generated by
  `scripts/make-marks.mjs` from `lib/brand-hues.json`: one tile shape,
  one stroke weight, one gradient treatment, a hue and a glyph of its
  own. `lib/brand.ts` hands the hue to cards, which use it twice: the
  deal card's art band and a faint wash on a Radar card. The platform's
  gold stays the only accent in a card body. Radar marks are served as
  `radar-<slug>.svg` so a locked member's image loads are identical for
