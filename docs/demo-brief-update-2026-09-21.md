...
- **The sticky bar, redesigned and quieted 2026-09-21.** Three groups of plain labels parted by hairlines: where (Open now, Radar), mine (Yours, For you), filter (Asset class, Industry). Whatever is chosen sits on one soft raised thumb that glides between options as you press or scroll; no gold rings, no filled pills. **Asset class** is one label on the same line; press it and the classes unroll to the right, the chosen one gets the thumb, and the industry menu stays beside them. Industry is a dropdown with a count per industry.
