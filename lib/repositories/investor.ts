...
async function seedPendingCommitment(userId: string, now: number): Promise<void> {
  ...
  const deadline = new Date(escrowDeadline(signedAt.getTime(), deal.targetClose));
  ...
  for (const entry of RECENT_BOOK) {
    ...
    const subscription = await prisma.subscription.create({
      data: {
        ...
        fundingDeadline: signed ? new Date(escrowDeadline(at.getTime(), deal.targetClose)) : null,
